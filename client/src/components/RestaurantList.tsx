...
const RestaurantList = () => {
    ...
    return (
        <div className="ml-10 grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-y-5 justify-center mt-10">
            {data.map((x: FormData ) => {
                //@ts-expect-error-some
                return <Card setTruDel={setTruDel} key={x.id} id={x.id} name={x.name} location={x.location} price={x.price_range} />;
            })}
        </div>
    );
};
//@ts-ignore-error
 const Card = ({name ,location , price , id , setTruDel }) => {
